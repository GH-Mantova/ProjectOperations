...
describe("Canonical CP-08 — pnpm seed is idempotent", () => {
  ...
  it("running the seed twice leaves every seeded table's row count unchanged", async () => {
    ...
    const counters: Record<string, () => Promise<number>> = {
      ...
      companyLegalDocument: () => prisma.companyLegalDocument.count(),
      companyProfile: () => prisma.companyProfile.count(),
      ...
      rateTable: () => prisma.rateTable.count(),
      rateColumn: () => prisma.rateColumn.count(),
      rateRow: () => prisma.rateRow.count(),
