...
| `apps/api/src/modules/jobs/job-number.service.ts` | G5 | Canonical J{YYMMDD}-{SLUG}-{NNN} generator. Per-client sequence computed from the jobs table, Brisbane TZ date stamp. |
| `apps/api/src/modules/jobs/dto/job-delivery.dto.ts` | G5 | CreateJobDto — `jobNumber` removed; always server-generated. |
| `apps/api/src/modules/jobs/dto/job-conversion.dto.ts` | G5 | ConvertTenderToJobDto — `jobNumber` removed (server-generated). ReuseArchivedJobConversionDto still declares it required because that path looks the archived row up by number. |
...
| `apps/api/src/modules/jobs/jobs.service.spec.ts` | G5 | Specs for the wider JobsService surface; convertTenderToJob specs (server-side generator, P2002 race 409). |
...
| `apps/api/prisma/seed-initial-services.ts` | (pre-B05) | Seeds J260315-QUEE-001 / J260328-BRIS-001. Upsert-by-id so the canonicalisation migration doesn't conflict on re-run. |
