...
1. List docs/pr-prompts/failed/ and diff against your own prior breadcrumbs (`docs/pr-prompts/00-03-machine-minder-*.md`, plus their copies under `archive/`), which are tracked and therefore read the same from a clone, from CI and from a cloud-fired station. Record one line per triaged file: filename -> verdict -> timestamp. Only triage NEW entries.
...
2. For each new failure, read the .log/.report.md tail AND pull the GitHub side: load connector tools via ToolSearch ("+github pull request"), find the failure's PR if one was opened (search_pull_requests by branch name), and use pull_request_read get_check_runs / get_status to read the actual CI verdicts instead of guessing from the local log. TRUST CI + THE ACTUAL DIFF OVER THE LOG'S OWN CLAIMS: if a .report.md says "fixed"/"done"/"passing", confirm it against the connector check-runs and `git diff origin/main --name-only` for that branch before believing it — watcher agents over-claim done (cf. #476 createPortal, #478 managerId DTO). Then classify:
   - KNOWN PATTERN (check FIRST, before writing any fresh diagnosis): consult sot/05-decisions-and-lessons.md (the incident ledger + operational playbooks) and match these recurring signatures — if it matches, apply/point to the documented remedy rather than re-diagnosing:
     ...
     * Watcher SILENT empty-run (exit 0 with no PR opened filed as success — routed to no-pr-opened/ after the #528 fix).
     Record "known-pattern: {name}" in your breadcrumb and, if a mechanical fix exists, stage the rev- fix pointing at the ledger playbook.
   - USAGE-LIMIT ("You've hit your limit - resets <time>"): parse the reset time (Australia/Brisbane local). Before reset: park the batch in your breadcrumb under a `## Parked` heading as "limit-parked until <time>", stop triaging further limit entries this run. After reset: restage ONE canary first (lowest-numbered rev- file, else lowest-numbered prompt) by COPYING it back to docs/pr-prompts/ with a fresh letter suffix before "-ready" (pr-190-...-ready.md -> pr-190b-...-ready.md; b taken -> c). Restage the rest only if the previous run's canary did not itself limit-fail. One wasted file beats ten.
   ...
   - AMBIGUOUS (cannot fully diagnose even with check-run data, or touches prisma migrations/seed/deploy/Azure/auth/destructive ops, or the same root cause already burned one failed fix attempt per your prior breadcrumbs): stage NOTHING; record for Marco.
...
4. Finish: write this run's breadcrumb (timestamp, files triaged, actions, any known-pattern hits), keeping a "## For Marco" section at the TOP (newest first), including "queue parked until <time>" when limit-parked.
...
- After any scripts/pr-watcher/** change merges to main, the running watcher still executes the
  ...
  the WRAPPER first, then the node, then relaunch DETACHED via C:\po-watcher\watcher-launcher-singlelane.ps1 (source of truth: `C:\po-watcher\ensure-watcher.ps1`, anchor `$Launcher =`; the non-singlelane wrapper exists on disk but the `PO Watcher Keepalive` task does not use it)
