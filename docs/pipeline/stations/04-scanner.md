...
1. docs/qa/qa-checklist.md — resume at the first [ ] or [~] item. If missing, rebuild from docs/qa/Master-QA-and-Consolidation-Program-Plan.md (Phase 0 + Workstream A actionable; B and C marked [-] BLOCKED). Ensure the checklist carries a recurring Part 0 static-audit item covering all six sub-checks; if absent, add it.
2. docs/qa/qa-findings.md — append-only. READ IT so you never re-file a known finding.
...
Record Part 0 findings in qa-findings.md like any other. Auth/prisma fixes are staged as prompts for Marco/shepherd review, never merged. A confirmed super-user redirect lockout is the one Part 0 case that may exceed visual-only severity — treat as S2; it counts toward your staged-prompt budget.
