...
Use the github-projectops connector (load via ToolSearch, e.g. "+github list pull request"). READS WORK, WRITES 403 — never attempt MCP writes. **You do not need to commit anything to do this audit, and you must not:** you are READ-ONLY on the board, your findings go in an UNTRACKED breadcrumb, and `scripts/pipeline/sweep-breadcrumbs.ps1` lands it in one PR. **Never `git commit` on `main`** — see NO-DRIFT in `docs/pipeline/stations/00-supervisor.md`. Each run:
