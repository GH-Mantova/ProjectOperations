---
premise: '! grep -rq "Ampol-Diesel-max" apps/api/src'
premise_means: The estimating Ampol-max-diesel fuel feed is not built (its marker fuelPriceSource="fuelpricesqld:Ampol-Diesel-max" is absent). Only token VALIDATION exists in api-keys.service.ts, not the daily estimating feed that writes OperationsSettings.fuelPricePerLitre.
scope:
  - apps/api/src/modules/estimates/**
  - apps/api/src/modules/admin-settings/**
  - packages/config/src/**
  - apps/web/src/pages/AdminSettingsPage.tsx
done_when: pnpm build && pnpm lint && grep -rq "Ampol-Diesel-max" apps/api/src
size: 7
gate_allow: env-vars
seed_only: false
escalates: false
...
# R3 T-2: fuelpricesqld.com.au live fuel-price feed (code now; activates when the token is entered)

REVIVED 2026-08-10 (Marco). Predecessors are on `main`: `pr-r3-t0-transport-capacity-fuel` (created
`OperationsSettings.fuelPricePerLitre` + `fuelPriceSource`/`fuelPriceFetchedAt`, #595) and
`pr-integration-keys-settings` (the `resolveIntegrationKey` seam, #593). So this is BUILDABLE NOW.
The feed only starts returning live prices once Marco enters the fuelpricesqld token in the ERP
**Integrations / API keys** settings — until then the env fallback + graceful "keep last value"
behaviour applies (no Azure step needed). Build the code now regardless.
...
tender's cost snapshot at rate-lock (T-1 already snapshots `fuelCost` per line). API spec confirmed
from the v1.5 PDF.
