...
Briefs **§2.2** (a Control-Panel style Settings "Home" with search) and **§2.3** (a plain-English
description for every setting). Same surface, run together. §2.1 was split out as its own brief.
...
- **Adjacent BACKLOG item:** `settings-restructure-sot-nav-reconcile` (SLICE 20, `sot/01` §9 nav-IA doc-reconcile) is gate-passed and READY. Settings Home changes nav IA, so the plan must state that the reconcile runs AFTER these slices, not before.
