...
**Provisioning** — `apps/api/src/modules/platform/sharepoint.service.ts:200`
...
**The copy guard** — `apps/api/src/modules/admin-imports/sharepoint-legacy-copy.service.ts:638`
...
is skipped — in `plan()` and again in the `execute()` re-check.
...
1. Add `site: { select: { name: true } }` to **both** Prisma `select` blocks in this
   file — the one in `plan()` that loads all tenders, and the one in `execute()` that
...
The literal `site: tender.site` is what kills the premise — keep that spelling.
...
- `project_name = NULL` and `site.name = "Acme Site"` → the guard probes
...
- `project_name` populated → `site.name` is ignored (projectName still wins).
- Both null/empty → falls back to the bare T-prefix, unchanged from today.
