...
- **Scope cards own discipline** (PR A2, 2026-05-16). `ScopeCard.discipline` is the authoritative discipline for a card and its child items (scope_of_works_items, scope_waste_items, cutting_sheet_items via `card_id` FKs). `ScopeOfWorksItem.discipline` continues to exist as a transitional column and is the read source for all services today, but is being migrated out in PR A2.5. New code reading discipline from a scope item should prefer `item.card?.discipline ?? item.discipline` until PR A2.5 lands, then `item.card.discipline` exclusively. The discipline → card-name + sortOrder defaults live in `apps/api/src/modules/tendering/scope/card-defaults.ts` (`SCOPE_CARD_DEFAULTS`, `getScopeCardDefault`) — both the seed and the A2 data migration use the same mapping. Cascade behaviour: deleting a tender deletes its cards (CASCADE on tenderId); deleting a card sets `card_id = NULL` on linked items (SetNull, preserves child data).
