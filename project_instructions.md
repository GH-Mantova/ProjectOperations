...
- **Scope item dimensions — hybrid derive contract** (PR B4a, 2026-05-17). Scope items carry seven dimension fields: four raw inputs (`length`, `height`, `depth`, `density`) and three derived-or-override slots (`sqm`, `m3`, `tonnes`). The pure helper `computeDerivedDimensions` lives in **two synchronised files**: `apps/api/src/modules/tendering/scope-item-dimensions.ts` (canonical) and `apps/web/src/pages/tendering/scopeItemDimensions.ts` (frontend mirror for live preview). Keep them byte-equivalent. Math: `sqm = explicit ?? length × height`; `m3 = explicit ?? sqm × depth`; `tonnes = explicit ?? m3 × density`. Frontend uses **controlled** inputs for these 8 fields (not the uncontrolled defaultValue pattern that Men/Days/Plant use) so we can distinguish "" (no explicit value — derive) from "0" (user typed zero — override). On blur of any one field, the frontend ships **all 7 dimension values** in a single PATCH; backend re-runs `deriveDimensionFields` to keep persisted sqm/m3/tonnes self-consistent. Raw fields (L/H/D/density) fall back to the existing row when the DTO didn't supply them. Override fields (sqm/m3/tonnes) only count as overrides when the DTO **actually sent them** — preventing stale derivations from freezing across partial patches. The DB cannot distinguish a derived sqm from a user override, so the contract is "frontend always sends the full picture on any dimension edit." Legacy `ScopeOfWorksItem.unit` + `value` are `@deprecated PR B4a` (retained for backward compat; cleanup PR drops them).
