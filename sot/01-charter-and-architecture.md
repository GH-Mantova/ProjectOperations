...
- **Runtime disk reads resolve via `__dirname`-relative bundled assets, never a repo-root walk** (Smart Wizard metadata-catalog incident, 2026-08-04; `docs/plans/smart-wizard-catalog-deploy-plan.md`, PRs #896/#904/#910; see `sot/05` LL-58). The deployed API artifact ships only `apps/api/` (no `docs/`/`scripts/`), so a service that reads a file at runtime resolves it env override -> build-bundled copy (`nest build` asset, `__dirname`-relative) -> repo-root walk (dev-only). A repo-root walk alone returns null in production and 503s.
...
One row per external integration. Answers, per surface: who is the system of record, what
data we hold vs what lives on the other side, how bytes flow between us, and what happens
when the other side is unreachable. Facts sourced from `apps/api/src/modules/**` and
`docs/qa/integration-idempotency-audit.md`. Cells marked `UNVERIFIED - Marco to confirm`
cannot be resolved from repo state today.

| Integration | System of record | Data owned by us vs by them | Ingestion strategy | Failure posture |
|---|---|---|---|---|
| **Xero** (accounting ledger) | Xero — canonical AR/AP ledger, GL posting, invoice numbering, payment application. | We own the source transaction (progress claim, `Expense`, `VendorInvoice`) and a local `xeroInvoiceId` / `xeroBillId` link on our row. Xero owns the ledger record and its numbering. | Outbound push, on-demand + retry: `syncContact`, `createInvoiceFromProgressClaim`, `pushBill`, `pushVendorInvoiceBill` called on user action or fire-and-forget from Expense APPROVE / 3-way match; `replayFailedBillPushes` cron every 30 min replays failures. Inbound pull: `syncPaymentStatus` cron every 6 h. No webhook. | Fire-and-forget pushes never fail the user's primary write (Expense/VendorInvoice ships even if Xero is down); failed pushes queue for the 30-min retry cron. On-demand endpoints (`POST /xero/sync-contacts`, `POST /xero/push-invoice`) surface Xero errors as `BadRequestException` — currently reflects raw upstream text (`sot/05` finding M1). Payment-status pull is best-effort; a missed 6 h cycle self-heals on the next run. |
| **SharePoint / Microsoft Graph mail** (document store + outbound email) | SharePoint owns the DOCUMENT BYTES (files under `SHAREPOINT_ROOT_FOLDER/[tender-or-project-id]/…`). The sending mailbox's Sent Items owns the SENT-MAIL artifact. | We own document METADATA (Prisma rows: path, name, size, uploader, audit trail) and the mail TRIGGER (which template fired, recipient list, correlation id). We do not read mail back. | Outbound only. SharePoint: on-demand `ensureFolder` / `uploadFile` from feature code via `graph-sharepoint.adapter.ts`. Mail: fire-and-forget calls through `EmailService.sendNotificationEmail` (Outlook provider) from contracts, projects, allocations, safety, compliance, tendering, procurement, client-quote send. No inbound webhook, no mailbox poll. | SharePoint upload failures propagate to the calling feature — the primary user write typically fails (see audit S1). Mail failures on notification paths are swallowed and logged (M1); on Client Quote send and PO issue the send failure is surfaced to the user. Two-way email reply parsing is on the planned list (§16), not built. |
| **Jotform** (external form ingestion) | UNVERIFIED - Marco to confirm — **no live Jotform integration in the repo**. Repo-wide grep of `apps/**` finds only inline comments naming a retired "Grice Office Key Checkout" form (superseded by `AssetCheckout`) and design-draft analysis docs. The live form-ingestion surface is our NATIVE forms engine, not Jotform. | N/A for Jotform. For the native forms engine: we own the submission, files, and every derived record end-to-end (`FormSubmission`, `FormFieldValue`, attachments in SharePoint). | N/A for Jotform (not wired). Native forms engine: `POST /forms/public/:token` (unauthenticated kiosk / QR) and `POST /forms/engine/:id/submit` (authenticated). | N/A for Jotform. Native forms engine idempotency and failure posture are documented in `docs/qa/integration-idempotency-audit.md` §F1–F3 (Forms v2 §4.4 bar: submission survives; delivery is logged; primary action never fails on a downstream integration hiccup). |
| **OTP email** (field-worker auth code delivery) | Us. The OTP hash, TTL, attempt counter, and verification result live in our DB via `OtpAuthProvider`. Email is a delivery channel; the mailbox is not queried. | We own the code, its hash, its expiry, and every verification attempt. The delivery provider sees only the plaintext code long enough to send it. | Outbound only, no inbound. Delivery goes through `OtpDeliveryPort`. Default binding is `LoggingOtpDelivery`, a dev/CI stub that LOGS the plaintext code (see `apps/api/src/modules/auth/otp-delivery.port.ts`). A production Microsoft Graph / SMTP adapter is UNVERIFIED - Marco to confirm — not yet wired (audit M6). | If delivery fails, the OTP row still exists — the field worker cannot receive the code and must re-request, burning an attempt against the request-side rate limit. Production posture (retry, quiet-fail vs surface-to-user) depends on the adapter Marco chooses; the port explicitly forbids the production adapter from logging the plaintext code. |
