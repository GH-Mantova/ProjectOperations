...
- **Failure honesty (MANDATORY)** (PR fix/no-access-page-instead-of-redirect, 2026-07-13). A permission failure must NEVER be silently redirected. Every capability-gated page in `apps/web/src` must render `<NoAccess required={...} />` when the required permission is missing — not `<Navigate to="/" />`, not blank, not the dashboard. The `<NoAccess />` component (in `apps/web/src/components/NoAccess.tsx`) names the exact permission code(s) required and tells the user to ask an administrator. **Why this rule exists:** on 2026-07-13 both Marco and Raj reported "Rates & Lists opens the dashboard instead" — the page was fine, they just lacked `rates.manage`. Because the redirect was visually indistinguishable from a broken page, we spent hours across multiple agents diagnosing the wrong theory (PR #537 supposedly not shipping). A page that simply said "You need `rates.manage`" would have ended the report in five seconds. Enforcement: CP-24 in `scripts/pr-gates/pr-gates.mjs` fails any PR that adds a `Navigate to="/"` line under `apps/web/src/pages/`. Legitimate non-permission redirects (unauthenticated → `/login`, canonical-URL rewrites, layout-level "wrong surface" redirects that keep the user's app usable) may opt out with `// eslint-ok: not-a-permission-redirect` on the same line — but the default answer is `<NoAccess />`. Rule 4 corollary: catch blocks in user-facing pages must not swallow errors entirely — surface something (toast, inline message, empty state with a reason). "Non-fatal — dropdown just renders empty" is fine when documented; a blank screen with no explanation is not.
