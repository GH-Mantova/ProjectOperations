...
test.describe("Batch 4 — Quotes (PRs #45, #46, #61, #62, #72, #242, #254, #256, #257)", () => {
  ...
  test("Generate Quote toggles the export panel; PDF + Excel downloads fire; history records both", async ({
    ...
    await expect(page.getByRole("heading", { name: "Generate quote" })).toBeVisible();
    await expect(page.getByText("Export history")).toBeVisible();

    // PR #61 — real PDF download event with the documented filename.
    ...
    // PR #45 — Excel export download event.
    ...
    // PR #45 — export history lists both rows (UI projection of
    // EstimateExport: type badge + "Generated by <user>" per entry).
    expect(await page.getByText(/Generated by/).count()).toBeGreaterThanOrEqual(2);
